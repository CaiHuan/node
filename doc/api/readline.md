...
* `options` {Object}
  ...
  * `postprocessor` {Function} An optional function used to post-process the
    current line before writing it to `output`.
...
function postprocessor(line) {
  // Highlight the current *input* line as JavaScript.
  return highlight('javascript', line);
}
```
